...
2. **Legacy framework removal plan** — files that WILL be removed: `.claude/commands/waves:*.md`, `.claude/hooks/waves-*.sh`, `ai_files/schemas/` (entire directory), and waves hook entries in `.claude/settings.json` (entries whose `command` references `waves-*.sh`).
3. **Display the full plan** (artifact moves + legacy removals, with counts + paths). State clearly: **every logbook's CONTENT is preserved untouched — only its file location is normalized; non-waves config is never touched.**
...
0. **Validation gate (BLOCKING — the migration is not "done" until this passes).** For every logbook RELOCATED in Step 3.5, run the deterministic schema validator (the same one logbook-create Step A6 uses): `python3 -m jsonschema -i logbooks/<slug>/logbook.json ${CLAUDE_PLUGIN_ROOT}/skills/waves-protocol/references/logbook_software_schema.json` (use `logbook_general_schema.json` for general projects). Since content was NOT touched, a logbook that was valid before stays valid — this gate confirms the relocation did not corrupt anything AND surfaces any logbook that was ALREADY invalid before migration (a pre-existing broken artifact). IF any relocated logbook fails validation → **STOP, report the failing logbook + field, and do NOT declare the migration complete.** git is the safety net — recoverable with `git restore` / `git reset`. (Known fields validate strictly; OPEN extras pass — design_principle #11.)
